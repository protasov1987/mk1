...
### Render (Blueprint)
- Файл `render.yaml` описывает веб‑сервис с нодой и диском для данных. По умолчанию сервис слушает порт, предоставленный Render, и
  привязывается к `0.0.0.0`.
- Чтобы развернуть проект, импортируйте репозиторий в Render и выберите режим **Blueprint**. Render автоматически подхватит `render.yaml`.
- Данные сохраняются на подключённом диске, смонтированном в каталог `data` внутри проекта (`/opt/render/project/src/data`).
